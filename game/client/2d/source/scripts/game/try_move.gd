...
func process_puzzle_blocks():
	...
	for blocks in game.puzzle_blocks:
		...
		if blocks.tile.x == x && blocks.tile.y == y:
			_block_count += 1
...
func try_move(dx, dy):
	...
	if game._player_holding_puzzle_block == true:
		if x < game.rooms[0].position.x + 1|| x > game.rooms[0].position.x + 13|| y < game.rooms[0].position.y + 1|| y > game.rooms[0].position.y + 13:
			return 
	...
	if x >= 0 && x < game.LEVEL_SIZE && y >= 0 && y < game.LEVEL_SIZE:
	...
	for mobs in game.mobs:
		if mobs.dead == false:
			...
			if mobs.respawn_position.x - mobs.tile.x > mobs._dead_at_tile_distance_from_start || mobs.tile.x - mobs.respawn_position.x > mobs._dead_at_tile_distance_from_start || mobs.respawn_position.y - mobs.tile.y > mobs._dead_at_tile_distance_from_start || mobs.tile.y - mobs.respawn_position.y > mobs._dead_at_tile_distance_from_start:
	...
	match tile_type:
		...
		Enum.Tile.Floor:
			...
			for blocks in game.puzzle_blocks:
				if blocks.tile.x == x - dx && blocks.tile.y == y - dy:
					blocks.move_puzzle_block(game, dx, dy, x, y)
				
			var blocked = false
			
			for mobs in game.mobs:
				if mobs.tile.x == x && mobs.tile.y == y && mobs.dead == false:
		...
		Enum.Tile.Door:
			...
			if game.map[x-1][y] == Enum.Tile.Ceiling || game.map[x+1][y] == Enum.Tile.Ceiling || game.map[x][y-1] == Enum.Tile.Ceiling || game.map[x][y+1] == Enum.Tile.Ceiling:
				for _i in range (game.ceiling.size()):
					# if there is a ceiling at this coordinates.
					if game.ceiling[_i].x == x-1 && game.ceiling[_i].y == y || game.ceiling[_i].x == x+1 && game.ceiling[_i].y == y || game.ceiling[_i].x == x && game.ceiling[_i].y == y-1 || game.ceiling[_i].x == x && game.ceiling[_i].y == y+1:
