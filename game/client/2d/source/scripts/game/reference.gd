...
class Mobs extends Reference:
	...
	func _init(game, mobs_level, x, y, _level_enemies_names, _total_enemy_at_level):
		...
		_str = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Strength + Settings._game.difficulty_level) - 1
		
		_def = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Defense + Settings._game.difficulty_level) - 1
		
		_con = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Constitution + Settings._game.difficulty_level) - 1
		
		_dex = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Dexterity + Settings._game.difficulty_level) - 1
		
		_int = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Intelligence + Settings._game.difficulty_level) - 1
		
		_cha = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Charisma + Settings._game.difficulty_level) - 1
		
		_wis = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Wisdom + Settings._game.difficulty_level) - 1
		
		_wil = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Willpower + Settings._game.difficulty_level) - 1
		
		_per = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Perception + Settings._game.difficulty_level) - 1
		
		_luc = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Luck + Settings._game.difficulty_level) - 1
		
		_des = Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].Description
		
		# starting hit points. Variable hp will be given to this var.
		_hp_max = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].HP + Settings._game.difficulty_level)
		
		# current hp. mobs dies once this value reaches 0.
		# _hp might not equal hp_max because mobs could spawn with low health.
		_hp = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].HP + Settings._game.difficulty_level)
		
		_mp_max = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].MP_max + Settings._game.difficulty_level)
		
		_mp = int(Json.d[str(Builder_playing._config.game_id)]["mobs"][_name].MP + Settings._game.difficulty_level)
		...
		_sprite_scene.texture = Filesystem._load_external_image(Variables._project_path + "/builder/objects/images/dictionaries/" + str(Builder_playing._config.game_id + 1) + "/mobs/" + str(Builder_playing._data.dungeon_number + 1) + "/" + str(Builder_playing._data.level_number + 1) + "/" + Json.d[str(Builder_playing._config.game_id)]["mobs"][_name]["Class"] + "/" + _name.to_lower() + "/1.png")
...
class Puzzle extends Reference:
			...
			if blocks.tile.x == x - dx && blocks.tile.y == y - dy && blocks._sprite_scene.get_node("Sprite").z_index == 100:
				...
				for yy in range (13):
					for xx in range (13):
						...
						if xx == _block_grabbed_at.x && yy == _block_grabbed_at.y && game._player_tile.x == x && game._player_tile.y == y:
	...
	func flip_dir(xx:float, yy:float, dx:float, dy:float, _commit = false):
		...
		while ((_row + xx >= -1) && (_row + xx <= 12) && (_column + yy >= -1) && (_column + yy <= 12)):
			...
			# The block playrer placed on the ground.
				...
				if _commit == false && _other_piece_found == true:
