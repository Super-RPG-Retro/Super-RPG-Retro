...
func _process(_delta):	
	if Variables._at_scene_sub != Enum.Scene_sub.Rune_buying:
		return
	...
	if Variables._num_first_rune_selected == -1 and _found_first_item == true:
...
func _input(event):	
	...
	if event is InputEventKey and event.pressed and Variables.select_json_dictionary_singly == true:
		if event.keycode == KEY_SPACE or event.keycode == KEY_ENTER or event.is_action_pressed("ui_left", true) or event.is_action_pressed("ui_right", true):		
			var _i = -1
			for _key in Json._magic.keys():	
				...
				if _check_button[_i].button_pressed == true and _button_buy.has_focus() == false and _item_list.has_focus() == false and _button_sort.has_focus() == false:
						_num_current = _i
	...
	if _selected_rune_name != "" and _button_buy.has_focus() == false and _item_list.has_focus() == false and _button_sort.has_focus() == false:
...
func _on_SpinBoxAmount_value_changed(value):
	...
	if _spinbox_amount.value * Json._magic[_selected_rune_name]["Gold"] > Hud._loaded.Gold and Json._magic[_selected_rune_name]["Stack_amount"] < 999:
...
func _on_ButtonSort_pressed():
	...
	if _current_item_selected[0] == 0:
		for _key in Json._magic.keys():
			...
			if _found_first_item == true and Variables._num_first_rune_selected == -1:
