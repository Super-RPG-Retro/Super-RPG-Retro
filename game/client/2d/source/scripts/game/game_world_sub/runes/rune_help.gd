...
func _input(event):
	if (event.is_pressed()):
		if (event.is_action_pressed("ui_escape", true)) and Variables._trigger_commands != Enum.Trigger_commands.Unit_description and Variables._rune_help.visible == true:
...
func _on_Arrow_left_data_input(event):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed or (event.is_action_pressed("ui_left", true)):
			_num -= 1 
...
func _on_Arrow_right_data_input(event):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed or (event.is_action_pressed("ui_right", true)):
			_num += 1 
