...
func build_level():
	...
	for x in range(game.LEVEL_SIZE):
		...
		for y in range(game.LEVEL_SIZE):
			...
			if Settings._system.show_stone_walls == true:
	...
	if Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x - 1, player_y)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x + 1, player_y)) == Enum.Tile.Floor or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x - 1, player_y)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x + 1, player_y)) == Enum.Tile.Ceiling or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x - 1, player_y)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x + 1, player_y)) == Variables._floor_rooms_tile_value:
		player_x += 1
		
	if Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y + 1)) == Enum.Tile.Floor or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y + 1)) == Enum.Tile.Ceiling or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(player_x, player_y + 1)) == Variables._floor_rooms_tile_value:
		player_y += 1
	...
	if Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) == Enum.Tile.Floor or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) == Enum.Tile.Ceiling or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) == Variables._floor_rooms_tile_value:
		_ladder_move_x += 1
		
	elif Enum.Tile.Door ==  game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y + 1)) == Enum.Tile.Floor or Enum.Tile.Door ==  game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y + 1)) == Enum.Tile.Ceiling or Enum.Tile.Door ==  game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y - 1)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x, ladder_y + 1)) == Variables._floor_rooms_tile_value:
		_ladder_move_y += 1
				
	elif Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) == Enum.Tile.Floor or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) == Enum.Tile.Ceiling or Enum.Tile.Door == game.tile_map.get_cell_source_id(0, Vector2i(ladder_x + 1, ladder_y)) and game.tile_map.get_cell_source_id(0, Vector2i(ladder_x - 1, ladder_y)) == Variables._floor_rooms_tile_value:
		_ladder_move_x -= 1
	...
	if Settings._game.room_ceiling == true and Settings._game.down_ladder_always_shown == false:		
	...
	for _i in range(num_mobs):
		...
		for mobs in game.mobs:    
			if mobs.tile.x == x and mobs.tile.y == y:
				blocked = true
				break
	...
	if Settings._system.remove_extra_stone_walls == true:
		...
		for x in range(game.LEVEL_SIZE):
			for y in range(game.LEVEL_SIZE):
				...
				if x > 0 and x < game.LEVEL_SIZE - 1 and y > 0 and y < game.LEVEL_SIZE - 1:
					if game.map[x][y-1] == Enum.Tile.Stone and game.map[x+1][y-1] == Enum.Tile.Stone and game.map[x+1][y] == Enum.Tile.Stone and game.map[x+1][y+1] == Enum.Tile.Stone and game.map[x][y+1] == Enum.Tile.Stone and game.map[x-1][y+1] == Enum.Tile.Stone and game.map[x-1][y] == Enum.Tile.Stone and game.map[x-1][y+1] == Enum.Tile.Stone:
				...
				if Settings._system.remove_level_border == true:
					if x == 0 or x == game.LEVEL_SIZE - 1 or y == 0 or y == game.LEVEL_SIZE - 1:
...
func _add_icons(_id):
	...
	while(_do_again == true):
		...
		if game.tile_map.get_cell_source_id(0, Vector2i(game.rooms[1].position.x + _x, game.rooms[1].position.y + _y)) == Enum.Tile.Floor or game.tile_map.get_cell_source_id(0, Vector2i(game.rooms[1].position.x + _x, game.rooms[1].position.y + _y)) == Enum.Tile.Floor_rooms:
			_do_again = false
			
			for icon in game.icons:
				if icon.tile.x == game.rooms[1].position.x + _x and icon.tile.y == game.rooms[1].position.y + _y: 
					_do_again = true
...
func clear_path(tile):
	...
	if Settings._game.keep_mobs_in_room == false:
		if tile.x > 0 and game.map[tile.x - 1][tile.y] == Enum.Tile.Floor:
		...
		if tile.y > 0 and game.map[tile.x][tile.y - 1] == Enum.Tile.Floor:
			points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x, tile.y - 1, 0)))
		
		if tile.x < game.LEVEL_SIZE - 1 and game.map[tile.x + 1][tile.y] == Enum.Tile.Floor:
			points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x + 1, tile.y, 0)))
		
		if tile.y < game.LEVEL_SIZE - 1 and game.map[tile.x][tile.y + 1] == Enum.Tile.Floor:
			points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x, tile.y + 1, 0)))
		
	if tile.x > 0 and game.map[tile.x - 1][tile.y] == Variables._floor_rooms_tile_value:
		points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x - 1, tile.y, 0)))

	# this time i will be checking down and to the right because these points might already be added.
	if tile.y > 0 and game.map[tile.x][tile.y - 1] == Variables._floor_rooms_tile_value:
		points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x, tile.y - 1, 0)))
	
	if tile.x < game.LEVEL_SIZE - 1 and game.map[tile.x + 1][tile.y] == Variables._floor_rooms_tile_value:
		points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x + 1, tile.y, 0)))
	
	if tile.y < game.LEVEL_SIZE - 1 and game.map[tile.x][tile.y + 1] == Variables._floor_rooms_tile_value:
		points_to_connect.append(game._mobs_pathfinding.get_closest_point(Vector3(tile.x, tile.y + 1, 0)))
	...
	for x in range(game.LEVEL_SIZE):
		for y in range(game.LEVEL_SIZE):
			if game.map[x][y] == Enum.Tile.Stone:
				...
				if x > 0 and game.map[x - 1][y] == Enum.Tile.Stone:
				...
				if y > 0 and game.map[x][y - 1] == Enum.Tile.Stone:
...
func add_random_connection(stone_graph, room_graph):
	...
	for position in path:
		...
		if end_room_id == 0 and Builder_playing._data.hide_random_corridor[Builder._config.game_id][Builder._data.dungeon_number][Builder._data.level_number] == 1 and Settings._system.show_stone_walls == false:
...
func add_room(free_regions):
	...
	if game._puzzle_room_dimension > 0 and game.room_number == 0:
	...
	for y in range(start_y + 1, start_y + size_y - 1):
		...
		for x in range(start_x + 1, start_x + size_x - 1):
			...
			if Settings._game.room_ceiling == false:
				...
				if game.room_number != 1 and game.visibility_map_saved.get_cell_source_id(0, Vector2i(x, y)) != -1:
...
func cut_regions(free_regions, region_to_remove):
	...
	for region in free_regions:
		if region.intersects(region_to_remove):
			...
			var _min_room_dimensions 
			if game._puzzle_room_dimension > 0 and game.room_number == 1:
				_min_room_dimensions = game._puzzle_room_dimension
			else:
				_min_room_dimensions = game._min_room_dimension
