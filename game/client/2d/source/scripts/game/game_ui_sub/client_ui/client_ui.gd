...
func _ready():
	...
	#_room_commands.custom_minimum_size.x = 640

func _input(event):	
	# if child scene is not open and there is a keyboard key pressed...
	if event is InputEventKey and event.pressed and _input_client.has_focus() == false:
		...
		if event.keycode == KEY_UP and _input_client.has_focus() == true:
	...
	if event.is_action_released("ui_focus_next"):
		if Variables._keyboard_tab_pressed == true:
			...
			if Settings._system.show_chat_features == true:
	...
	if	Variables._keyboard_tab_pressed == true and event.is_action_released("ui_right") == true:
	...
	if Variables._keyboard_tab_pressed == true and event.is_action_released("ui_up") == true:
		_tab_pressed = false
		Variables._keyboard_tab_pressed = false
		_focus_none()
	
	if Variables._keyboard_tab_pressed == true and event.is_action_pressed("ui_left") == true:
		if _input_client.has_focus() and _input_client.caret_column == 0:
			_tab_pressed = true
		...
		elif Settings._system.show_chat_features == true:	
...
func _on_button_game_pressed():
	...
	#_room_game.custom_minimum_size.x = 640
