...
func _input(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT and Input.get_current_cursor_shape() == CURSOR_ARROW:
		_clear_focus()
		
	if self.focus_mode == FOCUS_ALL and event.is_action_pressed("ui_up", true):
		_clear_focus()
...
func _on_text_entered(_text):
	...
	if text == "" or _client.get_node("Panel/VBoxContainer/Send/ButtonPanel/ListenButton").icon == load("res://2d/assets/images/host/host_connect.png") and text.substr(0, 1) != "/":
		return
	...
	# display a panel that has all the command keys that can be used.
		...
		Variables._at_scene_sub = Enum.Scene_sub.Game_help
		Variables._scene_title = "Game Help." 
		Variables._trigger_commands = Enum.Trigger_commands.Game_help
		_room_commands.text +=  "\n" + '-> Quick Help.'
	
	
	elif text == "/p0" or text == "/p1" or text == "/p2" or text == "/p3":
	...
	# rune
	elif text[0] == "/" and text.length() == 2 and text[1] == "r":
		_room_commands.text += "\n" + 'Unknown command, ' + text
		
		text = ""
	
	# rune select for casting.
	elif text[0] == "/" and text.length() == 4 and text[1] == "r" and text[2] == "c": 
	...
	# rune help.
	elif text[0] == "/" and text.length() > 2 and text.length() < 6 and text[1] == "r" and text[2] == "h": 
	...
	# rune buying
	elif text[0] == "/" and text.length() > 2 and text.length() < 6 and text[1] == "r" and text[2] == "b": 
		
		Variables._at_scene_sub = Enum.Scene_sub.Rune_buying
	...
	elif text == "/m" and text.length() == 2:
		Settings._system.music = !Settings._system.music
		
		if Settings._system.music == true and Variables._last_known_music_id == 6:
	...
	# get a max of 99 wait turns
	elif text[0] == "/" and text.length() > 2 and text[1] == "w":
		Variables._trigger_commands = Enum.Trigger_commands.Wait_a_turn
		...
		_room_public.text += "\n" + PC.character_stats[str(PC._number)]["_loaded"].Username + ": " + text
