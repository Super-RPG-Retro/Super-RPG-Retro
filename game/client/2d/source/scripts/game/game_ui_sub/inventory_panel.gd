...
func _process(delta):
	if _inventory_currently_selected != -1: 
		_output_text(Json._directory_number[str(Builder_playing._config.game_id)][Inventory._directory_name[_inventory_currently_selected]][Inventory._file_name[_inventory_currently_selected]]["Description"])
...
func _input(event):
	...
	if Variables._mouse_cursor_position.x > 657 and _inventory_currently_selected != -1:
	...
	if get_node("Inventory").visible == true:
		if event is InputEventMouseMotion:
			if get_node("Inventory/InventorySelect").visible == true and Variables._mouse_cursor_position.x <= 657:
	...
	if event is InputEventMouseButton:
		...
		if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT and _inventory_currently_selected != -1:
			Inventory.use(((Variables._inventory_current_group_selected - 1) * 24) + _inventory_currently_selected)
			
		
func player_stats_panel_size():
	if Settings._system.player_stats_panel_size == 2 or Settings._system.player_stats_panel_size == 3:
