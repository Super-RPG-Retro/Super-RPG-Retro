...
func _input(event):
	...
	if event is InputEventScreenTouch || event is InputEventMouseMotion:
...
func unit_description():
	...
	if Variables._child_scene_open == false && _last_known_tile_pos != _tile_pos || Variables._child_scene_open == false && Variables._type.size() == 0:
