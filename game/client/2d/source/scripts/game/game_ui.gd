...
func _process(_delta):
	...
	if Variables._game_over == true:
		if _game_over_dialog == null:
			...
			_game_over_dialog = Preload._game_over_dialog_scene.instance()
	...
	if Variables._trigger_escape_dialog == true:		 
		if _escape_dialog != null:
			...
			_escape_dialog = Preload._escape_dialog_scene.instance()
	...
	if Variables._rune_currently_selected == -1:
		_2d_map_main.get_node(_cursor).animation = "unit"	
...
func _input(event):
	...
	if event is InputEventMouseButton && event.button_index == BUTTON_RIGHT && event.is_action_pressed("ui_right_mouse_click") && Variables._rune_currently_selected != -1:
	...
	if event is InputEventMouseButton && event.is_action_pressed("ui_left_mouse_click") && Variables._child_scene_open == false && Variables._rune_currently_selected != -1 && _2d_map_main.get_node(_cursor).animation == "unit":
	...
	elif event is InputEventMouseButton && event.is_action_pressed("ui_left_mouse_click") && Variables._rune_currently_selected != -1 && _2d_map_main.get_node(_cursor).animation == "rune":
		get_tree().call_group("rune_casting", "rune_position_hide")
...
func rune_dialog_display(_text, _title):
	...
	_rune_casted_dialog.get_node("AcceptDialog").window_title = _title.replace("_", " ")
	_rune_casted_dialog.get_node("AcceptDialog").visible = true

func scene_3d():
	if Variables._at_library == true:
		if _3d == null:
			_3d = Preload._3d_scene.instance()
			add_child(_3d)
		...
		_2d_map_main.visible = false
		_2d_map_mini.visible = false
...
func add_scenes():
	if Variables._at_library == false:	
		if _2d_map_main == null:
			_2d_map_main = Preload._2d_map_main_scene.instance()
			add_child(_2d_map_main)
	...
	if _hud == null:	
		_hud = Preload._hud_scene.instance()
		add_child(_hud)
	_hud.visible = true
	
	
	if _client == null:
		_client = Preload._client_scene.instance()
		add_child(_client)
	_client.visible = true
	
	toggle_client_chat_panel()
		
	if _2d_map_mini == null:
	...
	if _artifacts == null:
		_artifacts = Preload._artifacts_scene.instance()
	...
	if _player_stats == null:
		_player_stats = Preload._player_stats_scene.instance()
		add_child(_player_stats)
	_player_stats.visible = true
		
		
	if _magic_panel == null:
		_magic_panel = Preload._magic_panel_scene.instance()
		add_child(_magic_panel)
	_magic_panel.visible = true


	if _inventory_panel == null:
		_inventory_panel = Preload._inventory_panel_scene.instance()
		add_child(_inventory_panel)
	_inventory_panel.visible = true

		
	if _tile_summary == null:
		_tile_summary = Preload._tile_summary_scene.instance()
	...
	if Variables._game_help == null:
		Variables._game_help = Preload._game_help_scene.instance()
...
func save_game():
	...
	_save_game_dialog = Preload._save_game_dialog_scene.instance()
...
func toggle_client_chat_panel():		
	...
	if Settings._system.small_client_panel == true:
		_client.get_node("MarginContainer").margin_top = 498
		_client.get_node(_client_size).rect_min_size.y = 84
		...
		_client.get_node(_client_size).rect_min_size.y = 164
		_client.get_node("MarginContainer/VBoxContainer/Client/Panel/VBoxContainer").rect_size.y = 164
		_2d_map_main.get_node("TextureRect").rect_position = Vector2( -365, -116)
...
func hide_parent_nodes():
	...
	_2d_map_main.get_node(_cursor).visible = false
	hide_tile_summary()
...
func tile_summary_input(event):
	...
	elif event is InputEventMouseButton && _2d_map_main.get_node(_cursor).visible == true && _2d_map_main.get_node(_cursor).animation == "unit":
		if event.button_index == BUTTON_LEFT && event.pressed && Variables._bypass_wait_a_turn == false:
			if Settings._system.sound == true:
				$Audio/SoundClick.play()
	
	# play a sound after selecting a spell when casting it at the main map.
	elif event is InputEventMouseButton && _2d_map_main.get_node(_cursor).visible == true && _2d_map_main.get_node(_cursor).animation == "rune":
		if event.button_index == BUTTON_LEFT && event.pressed && Variables._bypass_wait_a_turn == false:
			if Settings._system.sound == true:
				$Audio/SoundClick.play()
	...
	# do a wait turn or do action at puzzle block if at that tile.
	elif event is InputEventMouseButton && event.button_index == BUTTON_LEFT && !event.pressed && Variables._child_scene_open == false && Variables._bypass_wait_a_turn == false && Variables._mouse_cursor_position.x <= 529:
		Variables._wait_a_turn = 1
		get_tree().call_group("client", "_on_TimerGameTurn_timeout") 
			
	# go to the tile overview panel if right mouse button was clicked.	
	elif event is InputEventMouseButton && event.button_index == BUTTON_RIGHT && event.is_action_released("ui_right_mouse_click") && Variables._rune_currently_selected == -1 && Variables._child_scene_open == false && Variables._mouse_cursor_position.x >= -129 && Variables._mouse_cursor_position.x <= 529 && Variables._mouse_cursor_position.y >= -100 &&  Variables._mouse_cursor_position.y <= 284 && _display_overview == true:
		if Variables._child_scene_open == false:
			_2d_map_main.get_node("WorldViewport/GameWorld/Unit")._timer_object_coordinate.stop()
			_2d_map_main.get_node(_cursor).visible = false
	...
	if event is InputEventMouseMotion:
		...
		get_tree().call_group_flags(SceneTree.GROUP_CALL_REALTIME, "unit_description", "clear_i_node")
...
func tile_summary_overview():
	Variables._name.clear()
	Variables._type.clear()
	get_tree().call_group_flags(SceneTree.GROUP_CALL_REALTIME, "tile_summary", "unit_text_clear")
	...
	_tile_summary.get_node("ScrollContainer/GridContainer/Label").bbcode_text = ""
...
func cursor():
	...
	if Variables._mouse_cursor_position.x < -129 || Variables._mouse_cursor_position.x > 529 || Variables._mouse_cursor_position.y < -100 || Variables._mouse_cursor_position.y > 284 + int(Settings._system.small_client_panel) * 43 || Variables._wait_a_turn == - 1:
		...
		_2d_map_main.get_node(_cursor).visible = true
	
		if Variables._rune_currently_selected != -1:
			_2d_map_main.get_node(_cursor).animation = "rune"
...
func _on_main_tree_exiting():
	call_deferred("remove_child", _3d)
	call_deferred("remove_child", _2d_map_main)
	call_deferred("remove_child", _client)
	call_deferred("remove_child", _2d_map_mini)
	...
	call_deferred("queue_free", _3d)
	call_deferred("queue_free", _2d_map_main)
	call_deferred("queue_free", _client)
	call_deferred("queue_free", _2d_map_mini)
